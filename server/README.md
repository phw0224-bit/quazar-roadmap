...
- 사내 MCP 호출용 프로젝트/아이템/댓글 조회 및 생성·수정·삭제 API
...
| GET | `/api/mcp/projects/:projectId/activity` | 사내 MCP 전용 프로젝트 활동 조회. 현재 아이템 목록, 댓글 수, 티켓/브랜치 요약 포함 |
...
| GET | `/api/mcp/items/:itemId/comments` | 사내 MCP 전용 댓글 목록 조회. `boardType` 필요 |
| POST | `/api/mcp/items/:itemId/comments` | 사내 MCP 전용 댓글 생성. `boardType`, `content`, 선택 `tags`, 선택 `authorName` |
| PATCH | `/api/mcp/items/:itemId/comments/:commentId` | 사내 MCP 전용 댓글 수정. `content`, `tags`만 허용 |
| DELETE | `/api/mcp/items/:itemId/comments/:commentId` | 사내 MCP 전용 댓글 삭제. `boardType` 필요 |
...
- `COMMENT_NOT_FOUND`: 요청한 댓글이 없음
- `COMMENT_READ_ONLY`: GitHub 리뷰 시스템 댓글이라 수정/삭제 불가
