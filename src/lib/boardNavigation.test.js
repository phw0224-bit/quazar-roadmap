...
test('normalizeBoardType only allows known boards', () => {
  ...
  assert.equal(normalizeBoardType('team-overview'), TEAM_OVERVIEW_BOARD_TYPE);
...
test('resolveBoardTypeForBoardView falls back to the default team board', () => {
  ...
  assert.equal(resolveBoardTypeForBoardView('team-overview', 'AI팀'), TEAM_OVERVIEW_BOARD_TYPE);
...
test('board helpers expose the expected labels', () => {
  ...
  assert.equal(getBoardSectionLabel('team-overview'), '통합 팀 보드');
  assert.equal(getBoardSectionLabel('개발팀'), '개발팀 보드');
  assert.equal(isTeamBoard('개발팀'), true);
  assert.equal(isTeamBoard('team-overview'), true);
